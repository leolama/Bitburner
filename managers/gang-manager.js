//original from https://github.com/alainbryden/bitburner-scripts

import { formatMoney, formatNumberShort, getNsDataThroughFile, getActiveSourceFiles, runCommand, tryGetBitNodeMultipliers } from "./util.js";
...
const wantedPenaltyThreshold = 0.1; // Don't let the wanted penalty get worse than this
...
export async function main(ns) {
	...
	if (sf2Level == 0) return log(ns, "ERROR: You have not yet unlocked gangs. Script should not be run...");
...
async function initialize(ns) {
	...
	while (!(await getNsDataThroughFile(ns, "ns.gang.inGang()", "/data/player-gang-joined.txt"))) {
		if (!loggedWaiting) {
			log(ns, `Waiting to be in a gang. Will create the highest faction gang as soon as it is available...`);
			loggedWaiting = true;
		}
		await runCommand(ns, `${JSON.stringify(gangsByPower)}.forEach(g => ns.gang.createGang(g))`, "/data/gang-createGang.js");
		await ns.sleep(1000); // Wait for our human to join a gang
	}
	log(ns, "Collecting gang information...");
	const myGangInfo = ns.gang.getGangInformation(); //await getNsDataThroughFile(ns, 'ns.gang.getGangInformation()', '/data/gang-info.txt');
	...
	territoryNextTick = Date.now() + territoryTickTime; // Expect to miss by "caught unaware" by the first territory tick
	...
	requiredRep = -1;
	if (sf4Level == 0) log(ns, `INFO: SF4 required to get gang augmentation info. Defaulting to assuming ~2.5 million rep is desired.`);
	else {
		try {
			if (sf4Level < 3)
				log(
			...
			const augmentationNames = await getNsDataThroughFile(ns, `ns.getAugmentationsFromFaction('${myGangFaction}')`, "/data/gang-augs.txt");
			const ownedAugmentations = await getNsDataThroughFile(ns, `ns.getOwnedAugmentations(true)`, "/data/player-augs-purchased.txt");
			const dictAugRepReqs = await getDict(ns, augmentationNames, "getAugmentationRepReq", "/data/aug-repreqs.txt");
	...
	const equipmentNames = await getNsDataThroughFile(ns, "ns.gang.getEquipmentNames()", "/data/gang-equipment-names.txt");
	...
	allTaskNames = await getNsDataThroughFile(ns, "ns.gang.getTaskNames()", "/data/gang-task-names.txt");
	...
	myGangMembers = await getNsDataThroughFile(ns, "ns.gang.getMemberNames()", "/data/gang-member-names.txt");
...
async function mainLoop(ns) {
	...
	if (!isReadyForNextTerritoryTick && territoryTickDetected && Date.now() + updateInterval >= territoryNextTick) {
		...
		const otherGangInfo = await getNsDataThroughFile(ns, "ns.gang.getOtherGangInformation()", "/data/gang-other-gang-info.txt"); // Returns dict of { [gangName]: { "power": Number, "territory": Number } }
...
async function updateMemberActivities(ns, dictMemberInfo = null, forceTask = null) {
	...
	if (
		await getNsDataThroughFile(
			...
			"/data/gang-set-member-tasks.txt"
		)
	)
		log(
			ns,
...
async function optimizeGangCrime(ns, myGangInfo) {
	...
	let wantedGainTolerance =
		currentWantedPenalty < -1.1 * wantedPenaltyThreshold && myGangInfo.wantedLevel >= 1.1 + myGangInfo.respect / 1000 && myGangInfo.respect > 200
			? -0.01 * myGangInfo.wantedLevel /* Recover from wanted penalty */
			: currentWantedPenalty < -0.9 * wantedPenaltyThreshold && myGangInfo.wantedLevel >= 1.1 + myGangInfo.respect / 10000
			? 0 /* Sustain */
			: Math.max(myGangInfo.respectGainRate / 1000, myGangInfo.wantedLevel / 10); /* Allow wanted to increase at a manageable rate */
	const playerData = await getNsDataThroughFile(ns, "ns.getPlayer()", "/data/player-info.txt");
	...
	if (ownedSourceFiles[4] > 0) {
		try {
			factionRep = await getNsDataThroughFile(ns, `ns.getFactionRep('${myGangFaction}')`, `/data/gang-faction-rep.txt`);
		} catch {
	...
	for (let shuffle = 0; shuffle < 1000; shuffle++) {
		...
		shuffleArray(myGangMembers.slice()).forEach((member, index) => {
			...
			const bestTask =
				taskRates[0][optStat] == 0 || Date.now() - (lastMemberReset[member] || 0) < options["min-training-ticks"] * territoryTickTime
		...
		while (totalWanted > wantedGainTolerance && Object.values(proposedTasks).some((t) => t.name !== "Vigilante Justice")) {
			const mostWanted = Object.keys(proposedTasks).reduce(
				(t, c) => (proposedTasks[c].name !== "Vigilante Justice" && (t == null || proposedTasks[t].wanted < proposedTasks[c].wanted) ? c : t),
				null
			);
			const nextBestTask =
	...
	if (bestTaskAssignments != null && myGangMembers.some((m) => assignedTasks[m] !== bestTaskAssignments[m].name)) {
		...
		log(
			...
			`SUCCESS: Optimized gang member crimes for ${optStat} with wanted gain tolerance ${wantedGainTolerance.toPrecision(2)} (${elapsed} ms). ` +
				...
				`Rep: ${formatNumberShort(oldGangInfo.respectGainRate)} -> ${formatNumberShort(myGangInfo.respectGainRate)}, Money: ${formatMoney(
					oldGangInfo.moneyGainRate
				)} -> ${formatMoney(myGangInfo.moneyGainRate)}`
		);
		...
		if (
			...
			log(
				...
				`WARNING: Calculated new rates would be Rep:${formatNumberShort(optRespect)} Wanted: ${optWanted.toPrecision(3)} Money: ${formatMoney(optMoney)}` +
					`but they are Rep:${formatNumberShort(myGangInfo.respectGainRate)} Wanted: ${myGangInfo.wantedLevelGainRate.toPrecision(3)} Money: ${formatMoney(
...
async function fixWantedGainRate(ns, myGangInfo, wantedGainTolerance = 0) {
	...
	log(
		ns,
		`WARNING: Generating wanted levels (${lastWantedLevelGainRate.toPrecision(3)}/sec > ${wantedGainTolerance.toPrecision(
			3
...
async function doRecruitMember(ns) {
	...
	if (await getNsDataThroughFile(ns, `ns.gang.canRecruitMember() && ns.gang.recruitMember('${newMemberName}')`, "/data/gang-recruit-member.txt")) {
...
async function tryAscendMembers(ns) {
	...
	for (const member of myGangMembers) {
		...
		if (undefined !== (await getNsDataThroughFile(ns, `ns.gang.ascendMember('${member}')`, "/data/gang-ascend-member.txt"))) {
...
async function doUpgradePurchases(ns, purchaseOrder) {
	...
	if (
		await getNsDataThroughFile(
			ns,
			`${JSON.stringify(purchaseOrder)}.reduce((success, o) => success && ns.gang.purchaseEquipment(o.member, o.equipmentName), true)`,
			"/data/gang-upgrade-members.txt"
		)
	)
		log(
			ns,
			`SUCCESS: Purchased ${purchaseOrder.length} gang member upgrades for ${formatMoney(purchaseOrder.reduce((t, e) => t + e.cost, 0))}. (${orderSummary})`,
			"success"
		);
...
async function enableOrDisableWarfare(ns, myGangInfo) {
	...
	const otherGangs = await getNsDataThroughFile(ns, "ns.gang.getOtherGangInformation()", "/data/gang-other-gang-info.txt"); // Returns dict of { [gangName]: { "power": Number, "territory": Number } }
	...
	if (shouldEngage != myGangInfo.territoryWarfareEngaged) {
		log(
			...
			(warfareFinished ? "SUCCESS" : "INFO") +
				...
				(!warfareFinished
					? `Lowest win chance is ${(100 * lowestWinChance).toFixed(2)}% with ${lowestWinChanceGang} (power ${formatNumberShort(
		...
		await runCommand(ns, `ns.gang.setTerritoryWarfare(${shouldEngage})`, "/data/gang-set-warfare.js");
	}
}

// Ram-dodging helper to get gang information for each item in a list
const getGangInfoDict = async (ns, elements, gangFunction) => await getDict(ns, elements, `gang.${gangFunction}`, `/data/gang-${gangFunction}.txt`);
const getDict = async (ns, elements, nsFunction, fileName) =>
	await getNsDataThroughFile(ns, `Object.fromEntries(${JSON.stringify(elements)}.map(e => [e, ns.${nsFunction}(e)]))`, fileName);
